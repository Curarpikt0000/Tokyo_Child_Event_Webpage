...
class ChuoScraper(BaseScraper):
    def __init__(self, source_config: dict):
        super().__init__("中央区", source_config)
        self.events_path = source_config.get("events_path", "/a0022/kosodate/akachan_tengoku/event.html")
    ...
    def fetch(self) -> list[dict]:
        ...
        try:
            logger.info(f"正在爬取中央区官网: {url}")
            resp = self.get(url)
            if not resp:
                logger.warning(f"无法获取中央区官网页面: {url}")
                return []
