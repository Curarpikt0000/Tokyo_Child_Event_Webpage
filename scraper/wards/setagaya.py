...
class SetagayaScraper(BaseScraper):
    def __init__(self, source_config: dict):
        super().__init__("世田谷区", source_config)
        self.events_path = source_config.get("events_path", "/mokuji/kosodate/")
    ...
    def fetch(self) -> list[dict]:
        ...
        try:
            logger.info(f"正在爬取世田谷区官网: {url}")
            resp = self.get(url)
            if not resp:
                logger.warning(f"无法获取世田谷区官网页面: {url}")
                return []
